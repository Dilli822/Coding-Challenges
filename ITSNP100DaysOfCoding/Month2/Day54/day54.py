...
# numpy sorting
arr = np.array([7, 1, 6, 0, 4, 23])
print("array sorted using sort ", np.sort(arr))
# array sorted using sort  [ 0  1  4  6  7 23]

arr2 = np.array([False, True, False, True])
print("sorting the boolean values--->", np.sort(arr2))
#sorting the boolean values---> [False False  True  True]

arr3 = np.array([[1, 4, 0, 2, 5], [876, 23,13434, 3465, -2543]])
print("multi array sorting ", np.sort(arr3))
# multi array sorting  [[    0     1     2     4     5] [-2543    23   876  3465 13434]]

# bars in matplot vertical with different colors
y_points = np.array([1 , 3, 5, 7, 9])
x_points = np.array(["A", "B", "C", "D", "E"])
plt.title("showing the bars with matplot")
plt.xlabel("Classes of Alphabets")
plt.ylabel("numeric values")
plt.grid(axis="y")
plt.bar(x_points, y_points, color="green")
plt.show()