...
# searching in numpy
# returns the value whose index gets matched 
arr = np.array([1, 3, 56, 76])
x = np.where(arr == 45)
print("we use where to find the value in arr", x) # we use where to find the value in arr (array([], dtype=int64),)
y = np.where(arr == 3)
print("we use where to find the value in arr", y) # returns tuple (array([1]),) which means value is present at index 1

arry = np.array([1,2 ,3,4,5,6,6,6,7,1])
one = np.where(arr == 6)
print(one)

# specifying which grid lines to display
x1 = np.array([10, 20, 30, 40, 50, 60, 70, 80])
y1 = np.array([200, 300, 400, 600, 800, 560, 120, 200 ])

plt.xlabel("x axis")
plt.ylabel("y axis")
plt.title("Showing grid for specific axis")
plt.plot(x1, y1)
plt.grid(axis='x')
plt.grid(axis='y')
plt.show()