...
# Tokenize the text into words (assuming it's already preprocessed)
words = text.split()
...
plt.bar(common_words[:20], word_frequencies[:20], color='green')
plt.title('Top 20 Most Common Words in the Text')
plt.xlabel('Word')
...
def visualize_comparison(input_str, output_str):
    fig, ax = plt.subplots(figsize=(14, 7))
    ax.set_title('Comparison between Input and Output')
    ax.set_xticks([])
    
    # Iterate through each character and plot a bar
    for i in range(len(input_str)):
        color = 'green' if input_str[i] == output_str[i] else 'red'
        ax.bar(0, 1, bottom=i, width=0.5, color=color, align='center')
    
    ax.set_ylim(0, len(input_str))
    ax.set_xlim(0, 1)
    ax.set_yticks(range(len(input_str)))
    ax.set_yticklabels(input_str)
    plt.gca().invert_yaxis()
    plt.show()

# Display comparison between input and output sequences
for i, (x, y) in enumerate(dataset.take(2), start=1):
    print("\n\nEXAMPLE", i, "\n")
    print("INPUT")
    input_str = int_to_txt(x)
    print(input_str)
    print("\nOUTPUT")
    output_str = int_to_txt(y)
    print(output_str)
    print("\nVisualization:")
    visualize_comparison(input_str, output_str)