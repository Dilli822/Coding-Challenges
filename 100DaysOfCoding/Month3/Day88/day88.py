...
# Convert the probabilities to a list for visualization
probabilities_list = time_pred.numpy().tolist()

# Create a list of characters for the x-axis
characters = [idx2char[i] for i in range(len(probabilities_list))]

# Create a bar plot using Seaborn
plt.figure(figsize=(12, 6))
sns.barplot(x=characters, y=probabilities_list, palette="viridis")
plt.xlabel('Character')
plt.ylabel('Probability')
plt.title('Probability Distribution for Next Character Prediction')
plt.xticks(rotation=45)
plt.text(0.5, 1.05, "The graph visualizes character probabilities for the next sequence, indicating higher bars for more likely characters, aiding in understanding the model's predictions.",
...
# Create a list to hold the indices of predicted characters that fall within 'a' to 'z' or 'A' to 'Z'
indices_within_range = [idx for idx, char in enumerate(predicted_chars) if char.isalpha()]

# Get the characters falling within 'a' to 'z' or 'A' to 'Z'
predicted_chars_within_range = [predicted_chars[idx] for idx in indices_within_range]

# Plot
plt.figure(figsize=(10, 6))
plt.scatter(indices_within_range, [0] * len(indices_within_range), c='green', label='Predicted Characters within a-z/A-Z')
plt.xticks(range(len(predicted_chars_within_range)), predicted_chars_within_range)
plt.xlabel('Predicted Characters')
plt.ylabel('Count')
plt.title('Predicted Characters within a-z/A-Z')
plt.legend()
plt.show()
