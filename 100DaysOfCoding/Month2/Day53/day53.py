...
# -----SEARCHING IN NUMPY FROM THE RIGHT HAND OR LEFT HAND SIDE 
# ---- SEARCHING MULTIPLE DATAS AT ONCE USING SEARCH SORTED METHOD
arr = np.array( [ 0, 2, 4, 6, 8] )
x = np.searchsorted(arr, 0, side='right')
print("searching from the right side ", x)
print(x)
# gives 0 because if we sort the data from right side then data is inserted as
# 0
# 0
# 2
# 4
...
y = np.searchsorted(arr, 90, side="left")
print("searching from the left side ",y)
# give 5 because we sort the data from the left it would be 0,2,4,6,8,90
z = np.searchsorted(arr, [2, 4, 6])
print(z)

arr1 = np.array([80, 85, 90, 95, 100, 105, 110, 115, 120, 125])
arr2 = np.array([240, 250, 260, 270, 280, 290, 300, 310, 320, 330])

x_points = np.copy(arr1)
y_points = np.copy(arr2)
